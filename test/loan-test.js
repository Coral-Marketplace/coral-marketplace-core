...
describe('************ Loans ******************', ()  => {
  let market, marketContractAddress, loanContract, loanContractAddress, nft, 
    nftContractAddress, owner, borrower, lender, loanAmount, feeAmount, loanDuration, 
    ownerAddress, borrowerAddress, lenderAddress, reefToken, token1Id, token2Id, loan1Id, 
    loan2Id, royaltyValue, maxGasFee, deadline, item1Id, item2Id;
  ...
  before(async () => {
    ...
    const tx1 = await nft.connect(borrower).createToken('https://fake-uri-1.com', borrowerAddress, royaltyValue, false);
    const receipt1 = await tx1.wait();
		token1Id = receipt1.events[0].args[2].toNumber();

    const tx2 = await nft.connect(borrower).createToken('https://fake-uri-2.com', borrowerAddress, royaltyValue, false);
  ...
  it('Should create loan proposal', async () => {
    ...
    const loanApproved = await nft.connect(borrower).isApprovedForAll(borrowerAddress, loanContractAddress);
    if (!loanApproved) {
  ...
  it('Should only allow to unlist loan to creator', async () => {
    ...
    await throwsException(loanContract.connect(lender).unlist(loan1Id),
  ...
  it('Should fund loan', async () => {
    ...
    // Evaluate results
    expect(Math.round(endBorrowerBalance)).to.equals(Math.round(iniBorrowerBalance + formatBigNumber(loanAmount)));
    expect(endLenderBalance).to.lte(iniLenderBalance - formatBigNumber(loanAmount))
    ...
    expect(deadline).to.lt(new Date(new Date().getTime() + 90000)).to.gt(new Date(new Date().getTime() + 30000)); // +/- 30 secs. margin for timestamp calculation
  });


  it('Should not allow to unlist funded loan', async () => {
    await throwsException(loanContract.connect(borrower).unlist(loan2Id),
  ...
  it('Should not liquidate loan before deadline', async () => {
    ...
    await throwsException(loanContract.connect(lender).liquidate(loan2Id), 
  ...
  it('Should liquidate loan', async () => {
    ...
    expect(endLenderBalance).to.lte(iniLenderBalance).gt(iniLenderBalance - formatBigNumber(maxGasFee));
  ...
  it('Should not repay loan if value sent is not enough', async () => {
    ...
    await throwsException(loanContract.connect(borrower).repay(loan2Id, { value: loanAmount }), 
  ...
  it('Should repay loan', async () => {
    ...
    // Evaluate results
    expect(Math.round(endLenderBalance)).to.equals(Math.round(iniLenderBalance + formatBigNumber(loanAmount)) 
      + formatBigNumber(feeAmount));
    expect(endBorrowerBalance).to.lte(iniBorrowerBalance - formatBigNumber(loanAmount) - formatBigNumber(feeAmount))
      .gt(iniBorrowerBalance - formatBigNumber(loanAmount) - formatBigNumber(feeAmount) - formatBigNumber(maxGasFee));
    expect(iniTokenOwner).to.equal(loanContractAddress);
